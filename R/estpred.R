...
#'
#' prediction and estimation after estimating covariance parameters
#' @param theta: covariance parameters in order 1) nugget, 2) partial sill, 3) range these are on logit scale, and are converted to max*expit(theta[i]) internally where max is maxvar or maxrange
#' @param y: vector of response variable
#' @param X: design matrix of explanatory variables
#' @param distmat: matrix of Euclidean distance among all points
#' @param autocor_fun: geostatistical autocorrelation function that take distmat and range parameter as it arguments
#' @param maxvar: maximum variance for either partial sill or nugget effect
#' @param maxrange: maximum for the range parameter
#' @param family: family for the data model, one of either "poisson", "binomial", "negbinomial", "gamma", or "invgaussian"
#' Xp: design matrix of explanatory variables for prediction locations
#' @param dist_op: Euclidean distance between observed and prediction locations
#' @param dist_pp: Euclidean distance among prediction locations
#' @author Jay Ver Hoef
#' @rdname estpred
#' @export estpred
